...
def take_data(ti): #확장성을 위해 type 별 json 전달하도록 만들기
    ...
    mp4_list = [filename for filename in glob.iglob(os.path.join(video_root,'**/*'), recursive=True) if filename.lower().endswith('mp4')]#환경에 맞게 변경 (테스트용 경로)
    video_root = mp4_list[0]
    return video_root
    
def check_data(ti): #데이터 분류
...
with DAG(
    ...
    # 4. Save processed datetime # 시각화 부분
